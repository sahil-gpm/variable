...
const Codingarea = () => {
    ...
    useEffect(() => {
        ...
        const handleErrors = () => {
            showToast("We are facing connection issues, please rejoin", "🛜", '#262625', 12, "white")
            navigate("/room/enter-room")
        }
        ...
        socketReference?.on("joined-room", ({ members, joiningMemberName }) => {
            showToast(joiningMemberName === userData.firstname + " " + userData.lastname ? "You joined the room" : joiningMemberName + " joined the room", "✅", '#262625', 12, "white")
            setMembers(members)
        })
        ...
        socketReference?.on("disconnected", ({ leavingMemberName, members }) => {
            ...
            if (leavingMemberName !== userData?.firstname + " " + userData?.lastname) {
                showToast(leavingMemberName + " left the room", "❌", '#262625', 12, "white")
            }
        })
        ...
        return () => {
            ...
            if (!location.state.hostEmail) {
                onHostLeave()
                showToast("You ended the room for everyone", '❌', '#262625', 12, "white")
            } else {
        ...
        <>
            {
                popup && <motion.div drag dragConstraints={constraintRef} animate={{ y: 50 }} className="modals-and-popups absolute top-10  right-20 z-30 max-w-[40%]">
                    <Roommodal
                        mainText={"Hello"}
                        subText={"Leaving the meet as a host will close ❌ the room for everyone, do you still want to leave "}
...

            {
                sharePopup && <motion.div dragConstraints={constraintRef} drag animate={{ y: 50 }} className="modals-and-popups absolute top-10 right-20 z-30 max-w-[40%]">
                    <Sharemodal
                        ...
                        subText={"Invite your friends to this room 👀 and enjoy the experience of learning together 💪"}
            ...
            <div className='text-white min-h-[100vh] overflow-hidden bg-black flex' ref={constraintRef}>
                ...
                <div className="members-and-chat-wrapper h-[100vh] xl:w-[25%] lg:w-[30%] py-4 relative">
                    <div className='text-lg font-semibold px-5 text-start'>{members.length < 2 ? "You are the only one in the room 🙁" : "Your coding mates 🙂"}</div>
                    ...
                    <div className="other-controls absolute bottom-3 flex flex-col items-center w-full lg:px-2 xl:px-5 gap-3">
                        <Button text={"Copy code for room 🔐"} backgroundColor={"#262625"} width={"100%"} padding={12} borderRadius={5} onClick={() => {
                            navigator.clipboard.writeText(code.roomCode)
                            showToast("Code copied", "🔑", '#262625', 12, "white")
                        }} />
                        <Button text={"Copy written code 👨‍💻"} backgroundColor={"#262625"} width={"100%"} padding={12} borderRadius={5} onClick={() => {
                            if (!writtenCode.trim()) {
